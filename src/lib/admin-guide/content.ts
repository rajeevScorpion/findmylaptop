...
export const ADMIN_SCREEN_GUIDES: readonly AdminScreenGuide[] = [
  ...
  },
  {
    ...
    steps: [
      { title: "Keep scheduled automation paused", instruction: "Leave Paused on while preparing and testing rulebooks. Daily discovery and Catalog refresh are independent scheduled switches. Paused stops scheduled runs, while Refresh now and Discover now remain available for an intentional admin test." },
      ...
      { title: "Audit before searching", instruction: "Run Audit existing catalog. Scroll to Catalog decisions on this page and approve or reject every suggested course mapping or publication-review handoff. Existing published or unpublished laptops are reused before a marketplace search is allowed." },
      ...
      { title: "Test manually", instruction: "Use Refresh now, then Discover now. A green Discovery run completed message confirms that the job finished; it does not guarantee that a new candidate was created." },
      { title: "Locate every output", instruction: "First inspect Catalog decisions lower on Product Curation. Then open Research Queue for new Agent-curated candidates. Use Growth Agents > Recent Jobs for the research.discover_laptops job when diagnosing execution. If a candidate is approved, complete it under Laptops as an unpublished record." },
      { title: "Review before publication", instruction: "In Research Queue verify the target domain, gap reason, portfolio role, rulebook version, curation score, specifications, freshness, risks, and suggested courses. Approval creates an unpublished laptop; verify its recommendation copy, cautions, evidence, image, price, availability, and mappings before using the separate Publish action." },
    ...
    troubleshooting: [
      ...
      { symptom: "The green completion message appears but no candidate is visible.", response: "Completion only means the job ended successfully. Check Catalog decisions on Product Curation, then Agent-curated entries in Research Queue, and finally the research.discover_laptops entry under Growth Agents > Recent Jobs. An empty result can be correct when courses are covered, an unpublished laptop can fill the gap, the domain cap is reached, three decisions are pending, all products are duplicates, or no Amazon result satisfies the rulebook." },
